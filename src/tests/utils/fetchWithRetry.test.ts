...
describe('fetchWithRetry', () => {
  ...
  it('should call fetch with expected headers and options', async () => {
    nock(url, {
      ...
      .get('')
      .reply(200, responseMock.data) // set mocked response with expected headers
  ...
  it('should return a response when fetch succeeds', async () => {
    ...
    expect(await response.json()).toStrictEqual({ test: 'any-data' })
  })
  ...
  it('should retry fetch after a delay when fetch fails and retries are set', async () => {
    nock(url).get('').times(3).delay(100).replyWithError(new Error()) // set mocked delayed failed responses
